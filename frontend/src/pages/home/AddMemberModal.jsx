...
const AddMemberModal = ({ isOpen, onClose, onMemberAdded }) => {
  ...
  const validateForm = () => {
    ...
    // Image validations
    if (!memberImage) {
      newErrors.memberImage = "Member photo is required";
    } else if (memberImage.size > 5 * 1024 * 1024) {
      newErrors.memberImage = "Member photo must be less than 5MB";
    }
    
    if (!voucherImage) {
      newErrors.voucherImage = "Voucher image is required";
    } else if (voucherImage.size > 10 * 1024 * 1024) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Send request
      const response = await addMember(formDataObj);
      ...
      setErrors({ submit: error.response?.data?.message || "Failed to add member. Please try again." });
